...
parser.add_argument('--dataroot',type = str,default = './train_data/original',help = 'path to dataset')
...
parser.add_argument('--nEpochs',type = int,default = 2000,help = 'number of epochs to train.py for')
...
transform = transforms.Compose([transforms.RandomCrop(opt.high_resolution),
                                transforms.ToTensor()])
...
scale = transforms.Compose([transforms.ToPILImage(),
                            transforms.Resize(opt.high_resolution),
                            transforms.ToTensor(),
...
generator = Generator(16,opt.high_resolution,opt.scale_factor)
...
noise2 = torch.normal(mean = 0.5,std = 0.25,size = (opt.batchSize,3,opt.high_resolution,opt.high_resolution))
noise2[noise2 > 0.7] = 1;
noise2[noise2 <= 0.7] = 0  # 阈值0.8，二值化
low_res = torch.FloatTensor(opt.batchSize,1,opt.high_resolution,opt.high_resolution)
...
for epoch in range(2):
    ...
    for i,data in enumerate(dataloader):
        ...
        #print(generator_content_loss.item())
...
# Do checkpointing
torch.save(generator.state_dict(),'%s/generator_pretrain3.pth' % opt.out)
...
for epoch in range(opt.nEpochs):
    ...
    for i,data in enumerate(dataloader):
        ...
        generator_content_loss = content_criterion(high_res_fake,high_res_real) + 0.003 * content_criterion(
    ...
    # Do checkpointing
    torch.save(generator.state_dict(),'%s/generator_final3.pth' % opt.out)
    torch.save(discriminator.state_dict(),'%s/discriminator_final3.pth' % opt.out)
print('End training')
